#!/usr/bin/env python3
# peer.py â€” interactive multi-peer UDP chat with Lamport + Vector clocks
    ...
    def _maybe_initiate_once(self, started_at: float):
        ...
        if self.initiate_to:
            if self.initiate_to != self.name:
                self.send_chat(self.initiate_to, self.init_text or f"Hi {self.initiate_to} â€” are you there?")
            self.initiate_to = None
        elif self.initiate_broadcast:
            for to_name in list(self.peers.keys()):
                self.send_chat(to_name, self.init_text or f"Hi {to_name} â€” are you there?")
                time.sleep(0.05)
